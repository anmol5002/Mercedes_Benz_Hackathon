...
def radar_filter_valid_angles(points: np.ndarray,max_abs_angle: float = 60,debug: bool = False) -> np.ndarray:
    """
    ...
    print(type(points))
    mangles = ((90 - (np.arctan2(points[:, 0], points[:, 1]) / np.pi * 180)) + 180) % 360 - 180
    if debug:
        print(mangles.astype(np.int32))

    # Filter out the points that have invalid angles.
    valid = np.abs(mangles) <= max_abs_angle
    pointcount_before = len(mangles)
    pointcount_after = valid.sum()
    # points = points[valid, :]
    if debug:
        print('Total points: %d, valid points: %d' % (pointcount_before, pointcount_after))
        min_angle = np.max(np.abs(mangles))
...
class NuScenesExplorer:
    ...
    def map_pointcloud_to_image(self,
        ...
        if pointsensor['sensor_modality'] == 'lidar':
            ...
            iuiu=pc.points
            # print(iuiu)
            # iuiu= radar_filter_valid_angles(iuiu)
            # print(iuiu)
            iuiuiu=iuiu
            iu=iu[0:3]
            iuiu=iuiu[8:11]
        ...
        # iu_points=view_points(iu.points[:3, :], np.array(cs_record['camera_intrinsic']), normalize=True)
        ...
        iu_points=iu[:,mask]
        if pointsensor['sensor_modality'] != 'lidar':
            iuiu_points=iuiu[:,mask]
            iuiuiu_points=iuiuiu[:,mask]
        result=[[0 for j in range(2)]for k in range(len(points[0,:]))]
        if pointsensor['sensor_modality'] != 'lidar':
            for i in range(len(points[0,:])):
                result[i]=[points[0,:][i],points[1,:][i],iu_points[2,:][i],iu_points[0,:][i],iuiu_points[0,:][i],iuiu_points[1,:][i],iuiuiu_points[0,:][i],iuiuiu_points[1,:][i]]
        # a = np.asarray(iu_points[0,:])
        # b=np.asarray(iu_points[1,:])
        if pointsensor['sensor_modality'] != 'lidar':
            np.savetxt("/home/r3dg0li4th/data/aa.csv", result, delimiter=',')
    ...
    def render_pointcloud_in_image(self,
        ...
        # plt.imshow(im)
        g=np.asarray(points[0,:])
        # print(len(points))
        np.savetxt("/home/r3dg0li4th/data/cc.csv", g)
        g=np.asarray(points[1,:])
        np.savetxt("/home/r3dg0li4th/data/dd.csv", g)
